...
public class ClientUtil {
   public static <T extends BlockEntity> void renderBlock(BlockState state, MatrixStack matrices, VertexConsumerProvider vertexConsumers, T entity) {
      ...
      if (level != null) {
         MinecraftClient.getInstance().getBlockRenderManager().renderBlockAsEntity(state, matrices, vertexConsumers, getLightLevel(level, entity.getPos()), OverlayTexture.DEFAULT_UV);
      }
   ...
   public static <T extends BlockEntity> void renderItem(ItemStack stack, MatrixStack matrices, VertexConsumerProvider vertexConsumers, T entity) {
      World level = entity.getWorld();
      if (level != null) {
         MinecraftClient.getInstance().getItemRenderer().renderItem(stack, ModelTransformationMode.GUI, getLightLevel(level, entity.getPos()), OverlayTexture.DEFAULT_UV, matrices, vertexConsumers, level, 1);
      }
